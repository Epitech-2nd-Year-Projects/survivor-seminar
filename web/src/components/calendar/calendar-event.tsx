...
function getOverlappingEvents(currentEvent: CalendarEventType, events: CalendarEventType[]) {
  return events.filter((event) => {
    if (event.id === currentEvent.id) return false
    return currentEvent.start < event.end && currentEvent.end > event.start && isSameDay(currentEvent.start, event.start)
  })
}
...
function calculateEventPosition(event: CalendarEventType, allEvents: CalendarEventType[]): EventPosition {
  ...
  return { left, width, top: `${topPosition}px`, height: `${height}px` }
}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          className={cn(
            ...
            // theme-aware color classes (light/dark handled in config)
            (colorOptions.find((c) => c.value === event.color) ?? colorOptions[0]!)?.class.base ??
              "bg-blue-500/10 border border-blue-500 hover:bg-blue-500/20 text-blue-500",
            !month && "absolute",
            className,
          )}
          style={style}
        ...
        >
          <motion.div
            className={cn(
              ...
              // text color is already included in colorOptions.base via text-*
