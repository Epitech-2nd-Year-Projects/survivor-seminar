...
func RegisterNews(r *gin.RouterGroup, cfg *config.Config, db *gorm.DB, logger *logrus.Logger, uploader s3.Uploader) {
	h := v1handlers.NewNewsHandler(db, logger, uploader)
